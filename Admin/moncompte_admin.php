...
<body>
  ...
  if (isset($_POST['update']) && $_POST['update'] == 'Modifier') {
    ...
    if ($oldpwd == $result['password']) {
      ...
      if ($newpwd == '') {
        $upd = $bdd->prepare("UPDATE users SET nom = '".$nom."', prenom = '".$prenom."'");
        $upd->execute();
      }
      else {
        $upd = $bdd->prepare("UPDATE users SET nom = '".$nom."', prenom = '".$prenom."', password = '".$newpwd."'");
        $upd->execute();
      }
    }
  }
      ...
      <div class="modal-content">
        ...
        <div class="modal-body" id="modifInfos">
          <form action="moncompte_admin.php" method="post">
            ...
            <p>Nouveau mot de passe : <input type="password" name="newpwd" maxlength="100"></p>
