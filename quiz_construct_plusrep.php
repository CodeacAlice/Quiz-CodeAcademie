...
if (isset($_POST['submit']) && $_POST['submit'] == 'submit') {
	...
	$correctans = $searchgoodans->fetch();

	$itscorrect = 1; // On suppose que l'utilisateur a bien répondu, on modifiera si ce n'est pas le cas
	// Pour chaque réponse proposée, on regarde si elle a été cochée avec in_array et si elle aurait dû être cochée avec $correctans[repcorrect]
	// TODO: modifier la BDD pour y ajouter les colonnes repcorrect
	for ($i=1; $i <= 4; $i++) { 
		if ((in_array('reponse'.$i, $answers) && !$correctans['rep'.$i.'correct'])
			|| (!in_array('reponse'.$i, $answers) && $correctans['rep'.$i.'correct']))
			{$itscorrect = 0;}
	}
...
<form action="quiz_test.php?idquiz='.$idquiz.'&nq='.$nbnext.'" method="post">
	<table align="center">
		...
		<tr>
			<td></td>
			<td><input type="checkbox" name="choix[]" value="reponse1">'.$result['reponse1'].'</td>
		</tr>
		<tr>
			<td></td>
			<td><input type="checkbox" name="choix[]" value="reponse2">'.$result['reponse2'].'</td>
		</tr>
		<tr>
			<td></td>
			<td><input type="checkbox" name="choix[]" value="reponse3">'.$result['reponse3'].'</td>
		</tr>
		<tr>
			<td></td>
			<td><input type="checkbox" name="choix[]" value="reponse4">'.$result['reponse4'].'</td>
		</tr>
		<tr>
